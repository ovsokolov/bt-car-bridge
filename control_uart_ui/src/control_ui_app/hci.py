...
COMMAND_HCI_AUDIO_BUTTON = opcode(GROUP_HCI_AUDIO, 0x30)
...
# 0x06/0x07 are FFWD begin/end in this firmware API; Next/Previous are 0x0A/0x0B.
COMMAND_AVRC_CT_NEXT = opcode(GROUP_AVRC_CONTROLLER, 0x0A)
COMMAND_AVRC_CT_PREV = opcode(GROUP_AVRC_CONTROLLER, 0x0B)
COMMAND_AVRC_TG_CONNECT = opcode(GROUP_AVRC_TARGET, 0x01)
COMMAND_AVRC_TG_DISCONNECT = opcode(GROUP_AVRC_TARGET, 0x02)
COMMAND_AVRC_TG_TRACK_INFO = opcode(GROUP_AVRC_TARGET, 0x05)
COMMAND_AVRC_TG_PLAYER_STATUS = opcode(GROUP_AVRC_TARGET, 0x06)
COMMAND_AVRC_TG_REGISTER_NOTIFICATION = opcode(GROUP_AVRC_TARGET, 0x99)
...
TX_OPCODE_NAMES = {
    ...
    COMMAND_HCI_AUDIO_BUTTON: "HCI Audio Button",
    ...
    COMMAND_AVRC_TG_TRACK_INFO: "AVRCP TG Track Info",
    COMMAND_AVRC_TG_PLAYER_STATUS: "AVRCP TG Player Status",
    COMMAND_AVRC_TG_REGISTER_NOTIFICATION: "AVRCP TG Register Notification",
...
def encode_avrc_tg_track_info(attributes: list[tuple[int, str]]) -> bytes:
    payload = bytearray()
    for attr_id, text in attributes:
        if not (0 <= attr_id <= 0xFF):
            continue
        encoded = text.encode("utf-8", errors="ignore")
        if len(encoded) > 0xFF:
            encoded = encoded[:0xFF]
        payload.append(attr_id & 0xFF)
        payload.append(len(encoded) & 0xFF)
        payload.extend(encoded)
    return bytes(payload)


def encode_avrc_tg_player_status(play_state: int, song_len_ms: int, song_pos_ms: int) -> bytes:
    safe_state = play_state & 0xFF
    safe_len = max(0, min(song_len_ms, 0xFFFFFFFF))
    safe_pos = max(0, min(song_pos_ms, 0xFFFFFFFF))
    return bytes([safe_state]) + safe_len.to_bytes(4, "little") + safe_pos.to_bytes(4, "little")
