...
- Phase 2.1 correction: the UI must not semantically translate HCI events into `BR1,*` lines. Firmware owns bridge semantics; the UI is only the raw PUART relay until physical cross-wire is enabled.
...
- Current bridge images are `HFTEST13` and `AGTEST63`. `HFTEST13` consumes `BR1,ANSWER/REJECT/HANGUP/IGNORE` from PUART and owns phone-status TX: HFP events enqueue `BR1,INCOMING`, repeated `BR1,RING`, one-shot `BR1,CID,<number>`, `BR1,ACTIVE`, and `BR1,ENDED` to HF PUART. `AGTEST63` keeps AG receive/simulate behavior for `BR1,INCOMING/RING/CID/ACTIVE/ENDED`, but AG call display now uses the same model as `examples/mtb-example-btsdk-audio-gateway-master`: app-owned calls into `HCI_CONTROL_AG_COMMAND_SET_CIND` and `HCI_CONTROL_AG_COMMAND_STR`.
- Current direction: prefer tracked app code plus public/local HCI control APIs. For the 20706 btstack-v1 AG library, `HCI_CONTROL_AG_EVENT_CLCC_REQ` is only emitted to the HCI host, so `AGTEST63` adds one narrow tracked SDK adapter in `mtb_shared`: a weak `hci_control_ag_bridge_on_clcc_req(handle)` hook. The hook only records pending CLCC in app firmware; the existing AG app timer context sends the actual `+CLCC`/`OK` response. The patch is mirrored in `mtb_shared_overrides/headset_hfp_uart_ag/btsdk-audio-release-v4.9.5.patch`.
- For AG call display, `BR1,INCOMING/ACTIVE/ENDED` updates CIND state through `HCI_CONTROL_AG_COMMAND_SET_CIND`, and AG notifies the car with `+CIEV` through `HCI_CONTROL_AG_COMMAND_STR`. `BR1,RING` and `BR1,CID,<number>` also go through `HCI_CONTROL_AG_COMMAND_STR`.
- AGTEST62 call-initiation robustness: `BR1,RING` and `BR1,CID,<number>` assert incoming `callsetup=1` through the same HCI AG command path before sending `RING`/`+CLIP`, so the car display can recover if `BR1,INCOMING` was missed or arrived before the car HFP link was ready.
- AGTEST62 moves AG PUART semantic handling out of the PUART receive byte path. The RX path only queues complete `BR1,*` lines; the existing AG PUART flush timer logs `RX:` and then calls `hci_control_ag_bridge_handle_line()`. This avoids calling HFP AG/HCI command APIs from PUART interrupt/drain context.
- AGTEST62 deliberately removed the AGTEST57-59 experiments that forced SDK indicator bitmap state, used direct `hfp_ag_send_RING_to_hf()`, or sent unsolicited `+CLCC`. AGTEST63 keeps that cleanup and answers `AT+CLCC` only when the AG library emits `HCI_CONTROL_AG_EVENT_CLCC_REQ`. Expected HCI markers after the car asks `AT+CLCC`: `AG_CALL:CLCC req`, `AG_CALL:STR +CLCC: 1,1,4,0,0,"<number>",145`, `AG_CALL:STR OK`, and `AG_CALL:CLCC response`. It re-asserts `CIND 0,1,0,1,5,5,0` and `+CIEV: 2,1` on every `BR1,RING`/`BR1,CID`, because the official example requires indicator state before RING and the first indicator event can be missed/timed poorly.
- For car-originated commands, the desired design remains firmware-owned and symmetric: detect car `ATA/CHUP/CHLD` passively, queue `BR1,ANSWER/REJECT/HANGUP` to HF over PUART, and wait for HF/phone feedback (`BR1,ACTIVE/ENDED`) before AG changes call state. Do not locally answer/hang up or open SCO in AG when detecting car commands.
- Known limitation to investigate: the 20706 `COMPONENT_hfp_audio_gateway` API exposes `hfp_ag_startup(..., features)` with no app callback, while the newer btstack/PSoC Edge example exposes `wiced_bt_hfp_ag_startup(..., hfp_ag_event_callback)`. Without a callback, AG `HCI_CONTROL_AG_EVENT_AT_CMD` and `CLCC_REQ` may only be emitted to the HCI transport, not directly to app code.
- UI semantic PUART injection is disabled with `_semantic_puart_injection_enabled = False`; the old UI translation helpers are still present but should only log `Skipped UI semantic injection ...`. If Car PUART shows write timeouts during calls, first confirm the UI was restarted and this flag is false.
- `mtb_shared` remains vendor code, but `AGTEST63` has one accepted local SDK adapter for CLCC request notification on 20706 btstack-v1. Any local `mtb_shared` edit must be mirrored under `mtb_shared_overrides/` before commit; do not add parser-side PUART TX or answer/reject behavior there.
- Pairing/reconnect persistence must be firmware-owned, not UI-owned; the UI can relay PUART while testing, but it must not be required to replay NVRAM/link-key data for boards to reconnect
- HF firmware reconnect test image `HFTEST7` was flawed: `bt_hs_spk_control_reconnect_peer_bdaddr_get()` only reports the active reconnect target after reconnect has already started, so it cannot be used to decide whether a saved peer exists
- HF firmware reconnect test image is now `HFTEST8`: after `btheadset_post_bt_init()` succeeds, HF schedules a 2-second delayed reconnect check, makes BR/EDR pairable=no, discoverable=no, connectable=yes, checks saved link-key slot 0 with `bt_hs_spk_control_link_keys_get()` + `bt_hs_spk_control_misc_data_content_check()`, then calls `bt_hs_spk_control_reconnect()` and logs `AUTORECONNECT:start HFP_HF+A2DP_SINK` / `AUTORECONNECT:requested`; if no saved peer exists, logs `AUTORECONNECT:no bonded peer`
- HF firmware reconnect test image is now `HFTEST9`: keeps the HFTEST8 boot reconnect, and also registers `conn_status_change_cb`; BR/EDR link-down while the board remains powered starts a 10-second periodic firmware retry loop, BR/EDR link-up stops the retry loop. Expected markers include `AUTORECONNECT:BR_EDR link down retry scheduled`, `AUTORECONNECT:retry scheduled`, and `AUTORECONNECT:link restored stop retry`
- AG firmware reconnect test image is now `AGTEST2`: keeps the existing boot reconnect from saved NVRAM peer, registers `wiced_bt_dev_register_connection_status_change()`, and starts the same 10-second firmware retry loop when the BR/EDR peer drops while AG remains powered. Expected markers match HF, with reconnect attempts logged as `AUTORECONNECT:attempt HFP_AG+A2DP_SRC`
- AG firmware discovery/pairing test image is now `AGTEST3`: manual AG inquiry pauses firmware auto-reconnect retry so profile reconnect attempts do not collide with discovery, and inquiry emits `INQUIRY:start`, `INQUIRY:complete`, or `INQUIRY:start failed` over HCI bridge status. Classic legacy PIN pairing now sends `HCI_CONTROL_EVENT_PIN_REQUEST` to the UI and waits for `HCI_CONTROL_COMMAND_PIN_REPLY`; numeric comparison was already routed through `HCI_CONTROL_EVENT_USER_CONFIRMATION` / `HCI_CONTROL_COMMAND_USER_CONFIRMATION`
- AG reconnect is being returned to the older firmware-owned `wiced_app.c` staged reconnect model in `AGTEST16`: after SDP/profile init and local NVRAM restore, firmware enables connectable/discoverable/pairable mode, restores link keys from local NVRAM, then stages HFP AG, AVRCP TG, and A2DP source reconnect using the saved peer address. The UI may choose a device for manual pairing and display logs, but it must not own profile reconnect sequencing.
- For `AGTEST16`, `hci_control.c` service callbacks are intentionally log-only for HFP AG open/close so they do not start a second reconnect/A2DP timer. Manual bond/unbond cancels the `wiced_app.c` reconnect timer before starting a new pairing flow.
- Expected AGTEST16 HCI markers include `INIT:SDP registered`, `INIT:HFP_AG`, `BOOT:connectable pairable discoverable`, `PAIR:link keys save`, `PAIR:link key restore ok/fail`, `AUTO_AG:boot bonded peer found`, `AUTO_AG:stage HFP_AG`, `AUTO_AG:stage AVRCP_TG`, and `AUTO_AG:stage A2DP_SRC`.
- Historical AGTEST17/18 note: those images experimented with local SDK hooks in `mtb_shared` for event-gated reconnect and AG parser tolerance. That direction is retired for the current AGTEST52 app/HCI-API reset; do not reintroduce hidden SDK edits unless the limitation is documented and explicitly accepted.
- AGTEST19 pauses AG firmware auto-reconnect/profile chaining entirely for initial-connect debug. After pairing, firmware logs `AUTO_AG:disabled after pair`; boot reconnect also logs disabled. The UI Connect Selected flow now starts the initial AG profile connect after pairing, waits up to 18 seconds for `AG Service Open`, and only then attempts A2DP source. This isolates first-pair HFP service connection before revisiting reboot reconnect.
- AGTEST19 proved initial behavior: UI-driven post-pair AG profile connect opens HFP immediately, SLC completes, and A2DP source eventually connects after an early transient failure. AGTEST20 transfers only that proven initial HFP AG connect into firmware: after pairing success firmware queues a 1-second `hfp_ag_connect()` and logs `AUTO_AG:HFP only after pair`, `AUTO_AG:queue initial HFP_AG`, then `AUTO_AG:initial HFP_AG connect`. UI no longer starts AG profile connect after pairing, and A2DP remains manual/unchanged for now.
- Current confirmed checkpoint as of 2026-04-28 end of day: both boards reconnect to their paired peers automatically after reboot/power-cycle. HF reconnect work is committed as `5b2bdee Improve HF auto reconnect flow`; AG initial/reconnect profile work is committed as `d313326 Improve AG profile connection flow`. AG initial pairing now brings up both HFP AG and A2DP Source after accepting `WICED_BT_SDP_DB_FULL` as a usable SDP result when the Audio Sink record is present. Next planned project step is car-originated command handling over the existing UI-relayed PUART bridge path. Audio streaming/content tests are intentionally deferred because we do not yet have an actual audio test path worth validating.
