...
- A follow-up cleanup pass made `hci_control_audio.c` more sink-aware by selecting the peer A2DP service UUID from the runtime role and by retrying the HF-side A2DP reconnect stage a few times when the first SDP attempt after reboot fails.
...
- Pay close attention to whether the new `[HF_AUTO]` retry traces appear after AVRCP reconnect; if they do, the first A2DP SDP attempt is still racing the phone and the retry path is what should recover it.
