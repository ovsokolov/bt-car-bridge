...
static void av_app_send_setconfiguration( TIMER_PARAM_TYPE cb_params )
{
    ...
    {
        /* find a stream that is a source, is not in use, and is audio */
        if ( ( av_app_cb.sep_info[i].tsep == AVDT_TSEP_SRC ) &&
                                        ...
                                        wiced_bt_avdt_cfg_t *avdt_sep_config )
{
    ...
    for ( i = av_app_cb.sep_info_idx; i < av_app_cb.peer_num_seps; i++ )
    {
        ...
        if ( ( p_sep_info[i].tsep == AVDT_TSEP_SRC ) &&
...
static void av_app_disc_results (uint8_t handle, BD_ADDR bd_addr, uint8_t event, wiced_bt_avdt_ctrl_t *p_data)
{
    ...
    for (i = 0; i < p_discover->num_seps; i++)
    {
        ...
        /* find a stream that is a source, and is audio */
        if ((p_discover->p_sep_info[i].tsep       == AVDT_TSEP_SRC) &&
    ...
    WICED_BT_TRACE( "[%s]: source_cnt: = %d", __FUNCTION__, source_cnt );
    ...
    {
        WICED_BT_TRACE( "[%s] WARNING No Available Source SEPs on remote", __FUNCTION__ );
