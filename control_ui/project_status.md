...
- Car-originated HFP `AT+CHLD=0` is now translated to phone-side `AT+CHUP` for the two common single-call cases:
  - reject an incoming call
  - end an active non-held call
...
- The new `AT+CHLD=0` translation still needs real hardware validation against the car UI and phone-facing HF board.
...
- During an incoming and then an active call, confirm car-side reject/end now produces a phone-side hangup via bridged `AT+CHUP`.
