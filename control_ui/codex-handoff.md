...
- Car-side HFP `AT+CHLD=0` is now translated to phone-side `AT+CHUP` not only for incoming-call reject, but also for ending a normal active call when there is no held-call context.
...
- Validate car-screen reject and end actions during real calls and watch for the new bridge trace showing `AT+CHLD=0 -> AT+CHUP`.
